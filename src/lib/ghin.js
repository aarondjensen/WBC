...
//
//   searchGhinGolfers(query, state) → GET  /api/ghin?search=  (link: find a number)
...
//
// Ported from Bourbon Cup. WBC's relevance is specific: its handicap indexes
// are LOCKED for the tournament (see the handicap-edit guard in AdminView), so
// the one moment they are set is the moment they need to be right. Typing 16
// indexes off a phone screen is where they go wrong.

// GHIN encodes plus-handicaps as "+2.1" (a better-than-scratch player). The
// app models those as NEGATIVE indexes — calcCH and the stroke allocation in
// lib/individualBoard both read a negative index as "gives strokes back" — so
// convert on the way in. "NH" / blank / null → null (no handicap on file).
