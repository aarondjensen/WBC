...
import {
  ...
  FILES_PER_PROJECT, PHOTOS_PER_PROJECT, projectCapacity,
...
describe("archive naming", () => {
  it("derives a project and base url from the edition slug", () => {
    expect(archiveProject("2019")).toBe("wbc-photos-2019");
    expect(archiveBase("2019")).toBe("https://wbc-photos-2019.pages.dev");
  });

  it("gives every edition its own project", () => {
    const names = ["2009", "2019", "2026", "masters"].map(archiveProject);
...
describe("mediaDocId / paths", () => {
  it("scopes the document id to the edition", () => {
    expect(mediaDocId("2026", "abc")).toBe("med_2026_abc");
  });

  it("does not collide across editions for the same photo id", () => {
    expect(mediaDocId("2025", "abc")).not.toBe(mediaDocId("2026", "abc"));
  });

  it("puts both sizes under the edition's prefix", () => {
    expect(storagePaths("2026", "abc")).toEqual({
      full: "editions/2026/abc.webp",
      thumb: "editions/2026/abc_thumb.webp",
    });
  });
  ...
  it("carries the extension of whatever the browser actually encoded", () => {
    ...
    expect(storagePaths("2026", "abc", "jpg")).toEqual({
      full: "editions/2026/abc.jpg",
      thumb: "editions/2026/abc_thumb.jpg",
    });
  });

  it("mirrors the bucket layout in the archive so urls rebuild from an id", () => {
    expect(archivePaths("2019", "abc")).toEqual({
      full: "https://wbc-photos-2019.pages.dev/abc.webp",
      thumb: "https://wbc-photos-2019.pages.dev/abc_thumb.webp",
    });
  });

  it("keeps the two sizes as distinct files", () => {
    const p = storagePaths("2026", "abc");
...
describe("projectCapacity", () => {
  it("counts two files per photo against Cloudflare's cap", () => {
    const cap = projectCapacity(100);
    expect(cap.files).toBe(200);
    expect(cap.cap).toBe(FILES_PER_PROJECT);
  });

  it("fills a project at exactly half the file cap", () => {
    expect(PHOTOS_PER_PROJECT).toBe(FILES_PER_PROJECT / 2);
    expect(projectCapacity(PHOTOS_PER_PROJECT).over).toBe(false);
    expect(projectCapacity(PHOTOS_PER_PROJECT).remaining).toBe(0);
  });

  it("flags a deploy Cloudflare would refuse", () => {
    expect(projectCapacity(PHOTOS_PER_PROJECT + 1).over).toBe(true);
  });

  it("reports headroom for a typical edition", () => {
    expect(projectCapacity(500).remaining).toBe(9500);
    expect(projectCapacity(500).over).toBe(false);
  });
});
...
describe("takenTime", () => {
  it("prefers when the photo was taken over when it was uploaded", () => {
    const item = { takenAt: "2014-06-21T15:00:00.000Z", createdAt: "2026-08-06T12:00:00.000Z" };
    expect(takenTime(item)).toBe(Date.parse("2014-06-21T15:00:00.000Z"));
  });

  it("falls back to the upload time when there is no EXIF date", () => {
...
describe("sortByTaken", () => {
  const a = { id: "a", takenAt: "2014-06-21T15:00:00.000Z" };
  const b = { id: "b", takenAt: "2019-06-21T15:00:00.000Z" };
...
describe("groupByRound", () => {
  ...
  it("treats a whole archive with no rounds as one group, not as leftovers", () => {
...
describe("canDelete", () => {
  ...
  const archived = { host: HOSTS.pages, uploadedBy: "me" };
...
describe("ENCODINGS / MAX_UPLOAD_BYTES", () => {
  ...
  it("agrees with the cap written into storage.rules", () => {
    ...
    // If these drift, uploads fail as opaque permission errors.
...
describe("constants the rest of the app and the scripts share", () => {
  it("names the three hosts an item can live on", () => {
    expect(HOSTS).toEqual({ storage: "storage", pages: "pages", youtube: "youtube" });
  });
