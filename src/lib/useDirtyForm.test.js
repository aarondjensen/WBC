...
// detail, and a test that forced it into the public API to reach it would be
...
describe("stableStringify — the dirty comparison", () => {
  ...
  // Arrays are ordered data — a re-ordered pairing group is a different group.
  it("does NOT ignore array order", () => {
    expect(stableStringify({ g: ["a", "b"] })).not.toBe(stableStringify({ g: ["b", "a"] }));
  });

  it("treats the tournament form's shape consistently", () => {
    const a = { name: "WBC 2026", location: "Gaylord, MI", rounds: 4 };
    const b = { rounds: 4, location: "Gaylord, MI", name: "WBC 2026" };
    expect(stableStringify(a)).toBe(stableStringify(b));
    expect(stableStringify(a)).not.toBe(stableStringify({ ...a, rounds: 3 }));
  });
