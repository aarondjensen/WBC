...
const m = roundScoreId?.match(/*r(\d+)*/);
...
>>>>>>> 3fbf8748702f994e5391d81bc2eb1824a0ec1291
...
const scores = holeData[key] || {};
// Only count real scores (not WD sentinel 99) for net calculation
const realEntries = Object.entries(scores).filter(([*, s]) => s !== 99);
const thru = realEntries.length;
const isWDRound = Object.values(scores).some(s => s === 99);
if (thru > 0 || isWDRound) {
const gross = realEntries.reduce((a, [*, s]) => a + s, 0);
const parForHoles = realEntries.reduce((a, [h]) => a + (course.hole_pars[parseInt(h)] || 4), 0);
const ch = calcCH(p.handicap_index, slope, rating, par);
const netToPar = gross - parForHoles - ch;
if (!p.isWD) { roundsPlayed++; totalNetToPar += netToPar; totalThru += thru; }
...
<div style={{ minHeight: “100vh”, background: `radial-gradient(ellipse at 20% 50%, #0d1f3c 0%, ${K.bg} 70%)`, display: “flex”, alignItems: “center”, justifyContent: “center”, fontFamily: “‘Montserrat’, sans-serif”, fontVariantNumeric: “lining-nums tabular-nums”, padding: 20 }}>
<link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet" />
<style>{`@keyframes loginPulse { 0% { transform: scale(0.8); opacity: 0; } 50% { transform: scale(1.05); opacity: 1; } 100% { transform: scale(1); opacity: 1; } } @keyframes loginCheck { 0% { stroke-dashoffset: 24; } 100% { stroke-dashoffset: 0; } } @keyframes loginFade { 0% { opacity: 1; } 80% { opacity: 1; } 100% { opacity: 0; } } @keyframes toastUp { 0% { transform: translateX(-50%) translateY(20px); opacity: 0; } 100% { transform: translateX(-50%) translateY(0); opacity: 1; } } @keyframes shake { 0%,100% { transform: translateX(0); } 20%,60% { transform: translateX(-6px); } 40%,80% { transform: translateX(6px); } } @keyframes finalizeGlow { 0%,100% { box-shadow: 0 0 4px rgba(212,168,67,0.2); } 50% { box-shadow: 0 0 14px rgba(212,168,67,0.5); } } @keyframes pulse { 0%,100% { opacity: 0.5; } 50% { opacity: 0.2; } } @keyframes syncPing { 0% { transform: scale(1); opacity: 1; } 100% { transform: scale(2); opacity: 0; } }`}</style>
