...
import { fitPairings, rungLines, CARD_GAP } from "./lib/pairingsFit";
...
function GroupsView({ players, round, tRounds, courses, pairingsData, teeTimesData, getPlayerTee, user }) {
  ...
  const sizes = useMemo(
    () => groups.map(grp => grp.filter(pid => players.some(p => p.id === pid)).length),
    [groups, players],
  );
  ...
  // to read the layout again.
  const drawShape = sizes.join(",");
  ...
  const fit = fitPairings(box.h, sizes, box.w);
      ...
      {groups.length > 0 ? (
        ...
        <div ref={stackRef} style={{ display: "flex", flexDirection: "column", gap: CARD_GAP, flex: 1, minHeight: 0, overflowY: "auto" }}>
          {groups.map((grp, gi) => {
            ...
            const rows = grp.map(pid => players.find(pl => pl.id === pid)).filter(Boolean);
            return (
              ...
              {rows.map((p, pi) => {
                ...
                return (
                  <div key={p.id} style={{ padding: `${fit.rowPad}px 12px`, lineHeight: `${rowLine}px`, display: "grid", gridTemplateColumns: "5fr 1.6fr 2.4fr 2fr", alignItems: "center", borderBottom: pi < rows.length - 1 ? `1px solid ${K.bdr}${ALPHA.wash}` : "none", background: isMe ? K.t2 + ALPHA.wash : "transparent" }}>
                    ...
                    <span className="wbc-name" style={{ fontWeight: 600, fontSize: fit.name, color: isMe ? K.gold : K.t1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</span>
